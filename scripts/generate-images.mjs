// LitMatch character image generator
//
// Reads OPENAI_API_KEY from environment (load with: node --env-file=.env scripts/generate-images.mjs).
// Calls the OpenAI image generation API for each character and writes the
// PNG to public/characters/<id>.png. No SDK — uses built-in fetch.
//
// Style direction: Đông Hồ Vietnamese folk woodblock + classical scroll
// painting. The prompts intentionally over-specify Vietnamese cultural
// markers because gpt-image models otherwise default to Chinese tropes
// for "Asian historical figure".
...
const OUTPUT_DIR = resolve(PROJECT_ROOT, "public/characters");

// --- Style anchor ----------------------------------------------------------
//
// Re-used at the head of every prompt so the look stays consistent across
// characters. Mirrors the two reference images in mock style images/.
...
// --- Anti-Chinese guardrails ----------------------------------------------
//
// Reused in every prompt. Calls out specific failure modes the model has
// for "Vietnamese historical character".
...
// --- Per-character briefs --------------------------------------------------
//
// Each brief: visual subject from CSV "Image or illustration" column,
// expanded with concrete period markers and emotional tone from the bio,
// personality, and conflict fields. References the two mock-style images.

const characters = [
  {
    id: "chi-pheo",
    name: "Chí Phèo",
    work: "Chí Phèo (Nam Cao, 1941)",
    prompt: `
...
Mood: tragic, raw, the soul of a wronged peasant who once dreamed of being a "lương thiện" (decent) human being. Echoes the Đông Hồ folk style framing of the reference images, with botanical scroll ornaments around the edges.
  ...
  },
  {
    id: "mi",
    name: "Mị",
    work: "Vợ chồng A Phủ (Tô Hoài, 1952)",
    prompt: `
...
Mood: quietly resilient, sorrowful but with a flicker of awakening — she still has her youth, she still wants to live, even after years as a debt-slave daughter-in-law. Đông Hồ-style frame with botanical scroll ornaments.
  ...
  },
  {
    id: "xuan-toc-do",
    name: "Xuân Tóc Đỏ",
    work: "Số đỏ (Vũ Trọng Phụng, 1936)",
    prompt: `
...
Mood: satirical, theatrical, faintly absurd — the visual punchline of Vũ Trọng Phụng's social satire. Đông Hồ folk-print framing on the page.
  ...
  },
  {
    id: "luc-van-tien",
    name: "Lục Vân Tiên",
    work: "Lục Vân Tiên (Nguyễn Đình Chiểu, mid 19th century)",
    prompt: `
...
Mood: heroic, upright, the moral idealism of Nguyễn Đình Chiểu's Nam Bộ folk-poem hero.
  ...
  },
  {
    id: "thuy-kieu",
    name: "Thúy Kiều",
    work: "Truyện Kiều (Nguyễn Du, early 19th century)",
    prompt: `
...
if (!API_KEY) {
  console.error(
    "OPENAI_API_KEY not set. Run with: node --env-file=.env scripts/generate-images.mjs",
  );
  process.exit(1);
}

// Default per user instruction. Override with `IMAGE_MODEL=gpt-image-1 ...`
// if the account doesn't have access to gpt-image-2.
const MODEL = process.env.IMAGE_MODEL || "gpt-image-2";
const SIZE = process.env.IMAGE_SIZE || "1024x1536"; // portrait
const QUALITY = process.env.IMAGE_QUALITY || "high";

async function generate(character) {
  const fullPrompt = `${STYLE}\n\n${VN_GUARDRAILS}\n\nSubject: ${character.work}.\n\n${character.prompt}`;

  const start = Date.now();
  process.stdout.write(`[${character.id}] requesting ${MODEL} ${SIZE} ${QUALITY}... `);
  ...
  if (!res.ok) {
    ...
    throw new Error(`HTTP ${res.status} for ${character.id}: ${text}`);
  }
  ...
  if (!item?.b64_json) {
    throw new Error(
      `Unexpected response shape for ${character.id}: ${JSON.stringify(json).slice(0, 400)}`,
    );
  }

  const buf = Buffer.from(item.b64_json, "base64");
  const outPath = resolve(OUTPUT_DIR, `${character.id}.png`);
  await writeFile(outPath, buf);
  const ms = Date.now() - start;
  console.log(`saved ${outPath} (${(buf.length / 1024).toFixed(1)} KB, ${ms} ms)`);
}
...
async function main() {
  ...
  if (todo.length === 0) {
    console.error(`No character matches '${filter}'. Available IDs: ${characters.map((c) => c.id).join(", ")}`);
    process.exit(1);
  }

  console.log(
    `Generating ${todo.length} image(s) at ${SIZE} ${QUALITY} via ${MODEL}.`,
  );
  console.log(`Output: ${OUTPUT_DIR}\n`);

  // Run sequentially to avoid burst rate limits and keep output legible.
  const failures = [];
  for (const character of todo) {
    try {
      await generate(character);
    } catch (err) {
  ...
  console.log(
    `\nDone. ${todo.length - failures.length}/${todo.length} succeeded.`,
  );
  if (failures.length) {
